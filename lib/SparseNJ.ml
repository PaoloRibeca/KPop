    ...
    let compute ?(verbose = false)
      ...
      if verbose then
        Printf.eprintf "%s Sparse-NJ: built unrooted tree on %d leaves.\n%!"
          prefix n;
      root
    (* Direct merge-time bipartition emission, in parallel to [compute].
       Each merge of clusters i, j tracks an IntZ bitmask of the leaves
       under that cluster; the bipartition for each child of the merge
       (with cardinality > 1) is added to the result with weight = the
       NJ-computed branch length above that subtree.  This avoids the
       Trees.Newick build + Splits.of_newick traversal that
       [Splits.of_newick (compute ...)] would do, at the cost of
       leaving the final unrooted-edge bipartition unemitted: the
       merge loop stops at the trifurcating root, and the 3 root-edge
       bipartitions are then emitted explicitly at the end. *)
    let make_splits ?(verbose = false)
        ?(k_nn = 10) ?(row_sum = RowSum.Knn) ?(symmetry = Symmetry.One)
        names data =
      let open String.TermIO in
      let prefix = grey (Printf.sprintf "(%s):" __FUNCTION__) in
      ...
      if verbose then
        Printf.eprintf "%s Computing %d x %d distance matrix...\n%!" prefix n n;
      let d_mat = pairwise_distances data in
      let memberships = Array.make n IntZ.zero in
      for i = 0 to n - 1 do
        memberships.(i) <- IntZ.(one lsl i)
      done;
      let active = Array.make n true in
      let res = Trees.Splits.create names in
      let n_active = ref n in
      let emit_split mask weight =
        let card = IntZ.popcount mask in
        if card > 1 && card < n then
          Trees.Splits.add_split res (Trees.Splits.Split.of_intz mask) weight in
      while !n_active > 3 do
        let nbrs = knn_active d_mat active k_nn in
        let s = row_sums row_sum d_mat active nbrs !n_active in
        let cand = candidate_pairs symmetry nbrs active in
        ...
        let b_j = d_ij -. b_i in
        let b_i = max 0. b_i and b_j = max 0. b_j in
        emit_split memberships.(i) b_i;
        emit_split memberships.(j) b_j;
        memberships.(i) <- IntZ.logor memberships.(i) memberships.(j);
        for x = 0 to n - 1 do
          if x <> i && x <> j && active.(x) then begin
            let new_d = 0.5 *. (d_mat.(i).(x) +. d_mat.(j).(x) -. d_ij) in
            d_mat.(i).(x) <- new_d;
      ...
      (* Final three-way root: emit one bipartition per root-edge
         whose subtree has cardinality > 1.  This is the missing piece
         that the merge-time loop alone leaves out; without it, the
         splits set is short by one bipartition (the final unrooted
         edge between two complementary clusters) and Yggdrasill's
         reassembly produces a polytomy at the corresponding internal
         node. *)
      let active_three =
        let acc = ref [] in
        for i = n - 1 downto 0 do
      ...
      and b2 = max 0. (0.5 *. (d12 +. d23 -. d13))
      and b3 = max 0. (0.5 *. (d13 +. d23 -. d12)) in
      emit_split memberships.(i1) b1;
      emit_split memberships.(i2) b2;
      emit_split memberships.(i3) b3;
      if verbose then
        Printf.eprintf "%s Sparse-NJ: emitted %d internal splits.\n%!"
          prefix (Trees.Splits.cardinal res);
      res
  end: sig
