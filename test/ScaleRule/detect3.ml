(* detect3 <twisted.txt> <inertia.txt> <labels> <tag> <rungs,> <resamples> <labelled-only 0|1> [<z threshold, default 3>]
   detect3 --synthetic
...
let main () =
  ...
  let zmin = if Array.length Sys.argv > 8 then float_of_string Sys.argv.(8) else 3. and depth_min = 0.25 and share_lo = 0.002 and share_hi = 0.9 and ladder_hi = 0.5
  and dup = 0.015 in
  ...
  List.iter
    (fun d ->
      ...
      for i = 0 to n - 2 do
        ...
        for j = i + 1 to n - 1 do
          let b = pc.(j) in
          let s = ref (Float.Array.unsafe_get sq !id) in
          for k = !prev to d - 1 do
            let x = Array.unsafe_get a k -. Array.unsafe_get b k in
            s := !s +. (x *. x)
          done;
          Float.Array.unsafe_set sq !id !s;
          incr id
        done
      done;
      prev := d;
      let maxsq = ref 0. in
      Float.Array.iter (fun v -> if v > !maxsq then maxsq := v) sq;
      Float.Array.fill fh 0 fine 0.;
      let fw = sqrt !maxsq *. (1. +. 1e-9) /. float fine in
      Float.Array.iter
        (fun v ->
          let fb = int_of_float (sqrt v /. fw) in
      ...
      for i = 0 to n - 2 do
        let li = lab.(i) in
        for j = i + 1 to n - 1 do
          let b = int_of_float (sqrt (Float.Array.unsafe_get sq !id) /. w) in
